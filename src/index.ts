...
// Attempt to detect proxy using default endpoint
const checkProxyList: CheckProxy[] = [
...
const checkProxy = ([proxy, proxyName = ""] : CheckProxy): Promise<CheckProxy> => {
  console.log(`Check proxy[${proxyName}] at ${proxy}`);
  return fetch(proxy).then(() => [proxy, proxyName]);
};

Promise.race(checkProxyList.map(checkProxy)).then(([proxy, proxyName]: CheckProxy) => {
...
export const SYS_SETTINGS = {
  proxy: readFileSync(proxyFile) || process.env.HTTPS_PROXY || process.env.HTTP_PROXY,
};
...
const createWindow = (): void => {
  ...
  const url = MAIN_WINDOW_WEBPACK_ENTRY + `?useNativeMenuBar=${shouldUseNativeMenuBar ? "false" : "true"}`;
